...
fn read_bed_file(path: &str) -> Result<FnvHashMap<String, coitree::COITree<()>>, GenericError> {
    ...
    let now = Instant::now();
    let mut trees = FnvHashMap::<String, coitree::COITree<()>>::default();
    for (seqname, seqname_nodes) in nodes {
        trees.insert(seqname, coitree::COITree::new(seqname_nodes));
    }
    eprintln!("tree construction: {}s", now.elapsed().as_millis() as f64 / 1000.0);
